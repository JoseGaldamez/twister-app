...
func UploadBanner(repsonse http.ResponseWriter, request *http.Request) {
	file, handle, _ := request.FormFile("banner")
	...
	if err != nil || !status {
		http.Error(repsonse, "Error saving banner in DB"+err.Error(), http.StatusBadRequest)
		return
	}
